...
//const int ORBmatcher::HISTO_LENGTH = 50;
    ...
    int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint *> &vpMapPoints,
                                       const float th) // search Local Points
    {
        int nmatches = 0;
...
//        int bestDist=512;
            int bestLevel = -1;
            int bestDist2 = 256;
//        int bestDist2=512; // lch change here
            int bestLevel2 = -1;
            int bestIdx = -1;
...
//            const float dist = norm(MPdescriptor, d, cv::NORM_L1);
...
//     std::cout << "Matched points in local map: " << nmatches << endl;
    ...
    int ORBmatcher::SearchByNN(Frame &F, const vector<MapPoint *> &vpMapPoints) // searchLocalPoints
    {
...
//    MPdescriptors.create(MPdescriptorAll.size(), 32, CV_32F);
...
//    desc_matcher.match(MPdescriptors, F.mDescriptors, matches, cv::Mat());
    ...
    int ORBmatcher::SearchByNN(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches12) {
...
//L1
        ...
        int nmatches = 0;
//        int distance_num = 0;
//        cout << "LCH: the matches size is " << matches.size() << endl;
        for (int i = 0; i < static_cast<int>(matches.size()); ++i) {
//        int realIdxKF = matches[i].queryIdx;
//        int bestIdxF  = matches[i].trainIdx;

            const float dist = matches[i].distance;

//            cout << "LCH: distance is " << matches[i].distance << endl;
//            if (dist > TH_HIGH) // lch notice
//            {
//                distance_num++;
//                continue;
//            }
...
//     std::cout << "KeyFrame descriptors rows: " << pKF->mDescriptors.rows << std::endl;
//     std::cout << "CurrentFrame descriptors rows: " << F.mDescriptors.rows << std::endl;

        const vector<MapPoint *> vpMapPointsKF = pKF->GetMapPointMatches(); // GetMapPointMatches
...
//    std::cout << "desc_matches numbers: " << matches.size() << std::endl;
//    std::cout << "vpMapPointsKF numbers: " << vpMapPointsKF.size() << std::endl;

        int nmatches = 0;
        int bad_num = 0, none_num = 0, distance_num = 0;
        for (int i = 0; i < static_cast<int>(matches.size()); ++i) {
            int realIdxKF = matches[i].queryIdx;
            int bestIdxF = matches[i].trainIdx;

//        cout << "LCH: distance is " << matches[i].distance << endl;
            if (matches[i].distance > TH_HIGH) // lch notice
            {
                distance_num++;
                continue;
            }
...
//    std::cout << "distance_num is " << distance_num << std::endl;
//    std::cout << "bad_num is " << bad_num << std::endl;
//    std::cout << "none_num is " << none_num << std::endl;
...
# ifdef COMPILEDWITHL2
    ...
    int ORBmatcher::SearchByBoW(KeyFrame *pKF, Frame &F,
                                vector<MapPoint *> &vpMapPointMatches) // TrackWithKeyFrame, relocalise
    {
...
//                int bestDist1=256;
                    int bestIdxF = -1;
                    float bestDist2 = 768;
//                int bestDist2=256; // lch change here
...
//                    const float dist = norm(dKF, dF, cv::NORM_L1);
...
//            const int dist = (int)norm(dMP, dKF, cv::NORM_L1);
//            const float dist = norm(dMP, dKF, cv::NORM_L1);
...
//            const int dist = (int)norm(d1, d2, cv::NORM_L1);
//            const float dist = norm(d1, d2, cv::NORM_L1);
    ...
    int ORBmatcher::SearchByBoW(KeyFrame *pKF1, KeyFrame *pKF2,
                                vector<MapPoint *> &vpMatches12) //Loop Detection Compute Sim3
    {
        const vector<cv::KeyPoint> &vKeysUn1 = pKF1->mvKeysUn;
        const DBoW3::FeatureVector &vFeatVec1 = pKF1->mFeatVec;
        const vector<MapPoint *> vpMapPoints1 = pKF1->GetMapPointMatches();
...
//                    const int dist = (int)norm(d1, d2, cv::NORM_L1);
//                    const float dist = norm(d1, d2, cv::NORM_L1);
//                        cout << "LCH: the dist is " << dist << endl;
...
//                    if(bestDist1<500)
                    {
                        cout << "LCH: bestDist1 < TH_LOW" << endl;
                        if (static_cast<float>(bestDist1) < mfNNratio * static_cast<float>(bestDist2)) {
        ...
        cout << "LCH: nmatches before checkorientation is " << nmatches << endl;
...
//    cout << "LCH: SearchByPorjection begin" << endl;
//    cout << "LCH: the CurrentFrame mvScaleFactor is " << CurrentFrame.mvScaleFactors << endl;
        int nmatches = 0;
        ...
        for (int i = 0; i < LastFrame.N; i++) {
            ...
            if (pMP) {
                if (!LastFrame.mvbOutlier[i]) {
                    ...
                    int nLastOctave = LastFrame.mvKeys[i].octave; // 0
//                cout << "LCH: the nLastOctave is " << nLastOctave << endl;

                    // Search in a window. Size depends on scale
                    float radius = th * CurrentFrame.mvScaleFactors[nLastOctave];
//                float radius = th*CurrentFrame.mvScaleFactors[0];
...
//                    const float dist = norm(dMP, d, cv::NORM_L1);
//                    cout << "LCH: the dist is " << dist << endl;
...
// http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetParallel
    int ORBmatcher::DescriptorDistanceHamming(const cv::Mat &a, const cv::Mat &b)
    {
